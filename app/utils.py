...
class TextWrapper(textwrap.TextWrapper):
    ...
    def shorten(self, text):
        #self.width = width - 3
        wrapped_text = self.wrap(' '.join(text.strip().split()))#[0] + "..."
        if len(wrapped_text) > 1:
            shorten_text = wrapped_text[0] + "..."
        else:
            shorten_text = wrapped_text[0]
        #self.width = width
        return shorten_text
...
if __name__ == "__main__":
    ...
    string2 = "あいうえおかきくけこ"
    string3 = "abcdあいうえおかきくa"
    w = TextWrapper(width=6)
    print(w.wrap(string1), w.shorten(string1))
    print(w.wrap(string2), w.shorten(string2))
    print(w.wrap(string3), w.shorten(string3))
